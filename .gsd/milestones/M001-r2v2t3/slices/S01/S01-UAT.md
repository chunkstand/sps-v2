...
- Why this mode is sufficient: this slice’s success criteria are “real Postgres migrations apply and the schema is enforceable,” which is best proven by docker-compose + Alembic + integration tests.
...
4. **Expected:** Postgres starts successfully and migrations apply to a fresh volume.
...
2. **Expected:** Postgres container stays `Up` (does not exit during init). If it exits, check `docker compose logs postgres`.
