...
def test_persist_correction_task_creates_artifact(db_session):
...
@pytest.mark.integration
def test_persist_correction_task_idempotent(db_session):
...
@pytest.mark.integration
def test_persist_correction_task_validates_case_attempt_linkage(db_session):
...
@pytest.mark.integration
def test_persist_resubmission_package_creates_artifact(db_session):
...
@pytest.mark.integration
def test_persist_approval_record_creates_artifact(db_session):
...
@pytest.mark.integration
def test_persist_inspection_milestone_creates_artifact(db_session):
...
@pytest.mark.integration
def test_external_status_normalization_with_new_statuses(db_session):
...
@pytest.mark.integration
def test_approval_and_inspection_status_normalization(db_session):
