...
# M005 / S03 runbook verification: real intake API → fixture override worker
# → COMPLIANCE_COMPLETE → INCENTIVES_COMPLETE, with API read surfaces.
#
# What this does:
...
#   4) Reads fixture case IDs (phase4 + phase5) and clears fixture rows
#   5) POSTs /api/v1/cases and captures runtime case_id + project_id
#   6) Starts the worker with fixture override mapping
#   7) Asserts: INTAKE_COMPLETE → JURISDICTION_COMPLETE → RESEARCH_COMPLETE
#      → COMPLIANCE_COMPLETE → INCENTIVES_COMPLETE
#   8) Asserts: compliance_evaluations + incentive_assessments rows
#   9) GETs /api/v1/cases/{case_id}/compliance + /incentives
#
...
CASE_ID=""
PROJECT_ID=""
FIXTURE_CASE_ID_PHASE4=""
FIXTURE_CASE_ID_PHASE5=""
WORKER_PID=""
WORKER_LOG=""
API_PID=""
API_LOG=""
...
_http_post_json() {
  ...
  local url="$1"
  local body="$2"
  local out_file="${HTTP_RESPONSE_FILE:-/tmp/m005_s03_response_$$.json}"
...
API_LOG="$ROOT_DIR/artifacts/runbook/m005_s03_api_${RUNBOOK_TS}_$$.log"
...
# ---------------------------------------------------------------------------
# Phase 4: Read fixture case_id + clear fixture rows
# ---------------------------------------------------------------------------
...
if [[ -z "$FIXTURE_CASE_ID_PHASE4" || -z "$FIXTURE_CASE_ID_PHASE5" ]]; then
  echo "runbook.fail: missing_fixture_case_id" >&2
  exit 1
fi

echo "runbook: fixture_case_ids phase4_case_id=$FIXTURE_CASE_ID_PHASE4 phase5_case_id=$FIXTURE_CASE_ID_PHASE5" >&2

pg_assert_no_single_quotes "$FIXTURE_CASE_ID_PHASE4"
pg_assert_no_single_quotes "$FIXTURE_CASE_ID_PHASE5"
FIXTURE_QUOTED_PHASE4="$(pg_sql_quote "$FIXTURE_CASE_ID_PHASE4")"
FIXTURE_QUOTED_PHASE5="$(pg_sql_quote "$FIXTURE_CASE_ID_PHASE5")"
...
echo "runbook: clearing_fixture_artifacts phase4_case_id=$FIXTURE_CASE_ID_PHASE4 phase5_case_id=$FIXTURE_CASE_ID_PHASE5" >&2
pg_exec "delete from jurisdiction_resolutions where case_id=${FIXTURE_QUOTED_PHASE4}" >/dev/null
pg_exec "delete from requirement_sets where case_id=${FIXTURE_QUOTED_PHASE4}" >/dev/null
pg_exec "delete from compliance_evaluations where case_id=${FIXTURE_QUOTED_PHASE5}" >/dev/null
pg_exec "delete from incentive_assessments where case_id=${FIXTURE_QUOTED_PHASE5}" >/dev/null
...
HTTP_RESPONSE_FILE="$(mktemp /tmp/m005_s03_resp_XXXXXX)"
...
echo "runbook: fixture_override_enabled phase4_case_id=$SPS_PHASE4_FIXTURE_CASE_ID_OVERRIDE phase5_case_id=$SPS_PHASE5_FIXTURE_CASE_ID_OVERRIDE" >&2

# ---------------------------------------------------------------------------
# Phase 5: Start worker with fixture override
# ---------------------------------------------------------------------------

WORKER_LOG="$ROOT_DIR/artifacts/runbook/m005_s03_worker_${RUNBOOK_TS}_$$.log"
...
echo "runbook: starting_workflow_for_compliance case_id=$CASE_ID" >&2
...
async def main() -> None:
    ...
    await asyncio.wait_for(handle.result(), timeout=90.0)
...
echo "runbook: waiting_for_compliance_complete" >&2
_poll_ledger_for_state "$CASE_QUOTED" "COMPLIANCE_COMPLETE" 60

echo "runbook: waiting_for_incentives_complete" >&2
_poll_ledger_for_state "$CASE_QUOTED" "INCENTIVES_COMPLETE" 60
...
pg_assert_int_eq \
  "select count(*) from case_transition_ledger where case_id=${CASE_QUOTED} and event_type='CASE_STATE_CHANGED' and to_state='COMPLIANCE_COMPLETE'" \
  "1" \
  "case_transition_ledger.CASE_STATE_CHANGED.COMPLIANCE_COMPLETE"

pg_assert_int_eq \
  "select count(*) from case_transition_ledger where case_id=${CASE_QUOTED} and event_type='CASE_STATE_CHANGED' and to_state='INCENTIVES_COMPLETE'" \
  "1" \
  "case_transition_ledger.CASE_STATE_CHANGED.INCENTIVES_COMPLETE"
...
pg_assert_int_eq \
  "select count(*) from compliance_evaluations where case_id=${CASE_QUOTED}" \
  "1" \
  "compliance_evaluations.count(case_id)"

pg_assert_int_eq \
  "select count(*) from incentive_assessments where case_id=${CASE_QUOTED}" \
  "1" \
  "incentive_assessments.count(case_id)"

for comp_id in "${FIXTURE_COMPLIANCE_IDS[@]}"; do
  pg_assert_no_single_quotes "$comp_id"
  COMP_QUOTED="$(pg_sql_quote "$comp_id")"
  pg_assert_int_eq \
    "select count(*) from compliance_evaluations where compliance_evaluation_id=${COMP_QUOTED}" \
    "1" \
    "compliance_evaluations.id.${comp_id}"
done
...
for inc_id in "${FIXTURE_INCENTIVE_IDS[@]}"; do
  ...
  INC_QUOTED="$(pg_sql_quote "$inc_id")"
  pg_assert_int_eq \
    "select count(*) from incentive_assessments where incentive_assessment_id=${INC_QUOTED}" \
    "1" \
    "incentive_assessments.id.${inc_id}"
done
...
echo "runbook: fetching_api_compliance" >&2
HTTP_RESPONSE_FILE="$(mktemp /tmp/m005_s03_compliance_XXXXXX)"
export HTTP_RESPONSE_FILE
HTTP_STATUS="$(_http_get_json "${API_BASE}/api/v1/cases/${CASE_ID}/compliance")"
_assert_http_status "$HTTP_STATUS" "200" "get_compliance"
cat "$HTTP_RESPONSE_FILE"
echo

echo "runbook: fetching_api_incentives" >&2
HTTP_RESPONSE_FILE="$(mktemp /tmp/m005_s03_incentives_XXXXXX)"
export HTTP_RESPONSE_FILE
HTTP_STATUS="$(_http_get_json "${API_BASE}/api/v1/cases/${CASE_ID}/incentives")"
_assert_http_status "$HTTP_STATUS" "200" "get_incentives"
cat "$HTTP_RESPONSE_FILE"
echo
...
pg_print "select compliance_evaluation_id, evaluated_at from compliance_evaluations where case_id=${CASE_QUOTED}" >&2
pg_print "select incentive_assessment_id, assessed_at from incentive_assessments where case_id=${CASE_QUOTED}" >&2
